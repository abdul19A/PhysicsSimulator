    ...
    : position(pos), normal(Vector3Normalize(norm)), size(sz), frictionCoef(friction), bounceCoef(bounce)
{
    ...
    posNormProduct = Vector3DotProduct(normal, position);
}
...
float Wall::WallCollideBall(Ball& ball, float dt) const {
    ...
    float curNormProduct = Vector3DotProduct(p0, normal);
    float velNormProduct = Vector3DotProduct(ball.velocity, normal);
    float nextNormProduct = curNormProduct + velNormProduct * dt;

    if (curNormProduct * nextNormProduct >= 0) return dt; // root out non collisions
    if (std::abs(velNormProduct) < 1e-6f) return dt; // avoid /0

    float t = std::clamp((posNormProduct - curNormProduct) / velNormProduct, 0.0f, dt);

    // apply restitution and friction
    ball.velocity = Vector3Add(
